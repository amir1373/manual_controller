...
bool delta::asda::ServoHW::get_error_message(const uint16 &error_code, std::string &error_message)
{

  auto find_error_message = ethercat::error_codes.find(error_code); 
  
  if (find_error_message == ethercat::error_codes.end())
  {
    error_message = "";
    return false;
  }
  ...
  {
    error_message = const_cast<char *>(find_error_message->second.c_str());
    return true;
  }
