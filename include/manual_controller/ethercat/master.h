...
#include "manual_controller/ethercat/registry_idx.h"
#include "manual_controller/ethercat/pdo.h"
#include <stdio.h>
...
private:
  ...
  {
    printf("Slave Size: %ld\n", slaves.size());
    for (int i = 0; i < slaves.size(); i++)
    {
      const uint16 slave_idx = 1 + i;
      printf("Slave name: %s\n", ec_slave[slave_idx].name);
      printf("InputSlave: %s\n", slaves[i].c_str());
      // TODO: if (strcmp(ec_slave[slave_idx].name, slaves[i].c_str())) this is correct if
      // if (strcmp(ec_slave[slave_idx].name, ec_slave[slave_idx].name))
